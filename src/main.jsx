...
function Header({ page, setPage }) {
  ...
  return (
    <header className="site-header">
      ...
      <div className="desktop-actions"><button className="ghost-link" onClick={() => go('signin')}>Sign in</button><button className="btn btn-dark" onClick={() => go('signup')}>Join Deur</button></div>
      <button className="mobile-menu" onClick={() => setOpen(!open)} aria-label="Open menu">{open ? <X size={22} /> : <Menu size={22} />}</button>
      {open && <div className="mobile-panel">{navItems.map(item => <button onClick={() => go(item.page)} key={item.page}>{item.label}</button>)}<button onClick={() => go('signin')}>Sign in</button><button className="btn btn-dark" onClick={() => go('signup')}>Join Deur</button></div>}
    </header>
  );
}
...
function Marketplace({ setPage, full = false }) {
  ...
  return <section className="marketplace section-shell section-block"><div className="marketplace-top"><div><span className="kicker">Fiverr-inspired browsing</span><h2>{full ? 'Explore trusted services' : 'Popular trusted services'}</h2></div><button onClick={() => setPage('client-dashboard')} className="text-link">Post a job <ChevronRight size={18} /></button></div>{full && <div className="wide-search"><Search size={20} /><input placeholder="Search errands, assistants, designers, delivery, property help..." /></div>}<div className="filters">{filters.map(item => <button className={filter === item ? 'active' : ''} onClick={() => setFilter(item)} key={item}>{item}</button>)}</div><div className="gig-grid">{visible.map(gig => <GigCard gig={gig} setPage={setPage} key={gig.title} />)}</div></section>;
}
...
function AuthPage({ type, setPage }) { const isSignup = type === 'signup'; return <section className="auth-page section-shell"><div className="auth-copy"><span className="kicker">{isSignup ? 'Join Deur' : 'Welcome back'}</span><h1>{isSignup ? 'Create your Deur account.' : 'Sign in to Deur.'}</h1><p>{isSignup ? 'Choose whether you want to post gigs as a client or earn money as a provider. Real auth comes next with Supabase.' : 'This sign-in screen is ready for Supabase authentication in the next phase.'}</p><div className="auth-benefits"><div><ShieldCheck /> Verified marketplace</div><div><WalletCards /> Escrow protection</div><div><MessageCircle /> Secure messaging</div></div></div><form className="auth-card" onSubmit={(e) => { e.preventDefault(); setPage(isSignup ? 'client-dashboard' : 'client-dashboard'); }}><img src="/deur-logo.svg" alt="Deur logo" /><h2>{isSignup ? 'Start with Deur' : 'Log in'}</h2>{isSignup && <div className="role-tabs"><button type="button">I need help</button><button type="button">I want gigs</button></div>}<label>Full name<input placeholder="Your name" /></label><label>Email<input type="email" placeholder="you@example.com" /></label><label>Password<input type="password" placeholder="••••••••" /></label><button className="btn btn-green" type="submit">{isSignup ? 'Create account' : 'Sign in'}</button><p className="form-note">Demo UI only. We’ll connect real login with Supabase next.</p><button type="button" className="link-button" onClick={() => setPage(isSignup ? 'signin' : 'signup')}>{isSignup ? 'Already have an account? Sign in' : 'New here? Join Deur'}</button></form></section>; }
...
function App() {
  ...
  const render = () => {
    ...
    if (page === 'signin') return <AuthPage type="signin" setPage={setPage} />;
  ...
  return <><Header page={page} setPage={setPage} /><main>{render()}</main><Footer setPage={setPage} /></>;
}
