...
function Marketplace({ setPage, full = false }) {
  ...
  return <section className="marketplace section-shell section-block"><div className="marketplace-top"><div><span className="kicker">Fiverr-style + local services</span><h2>{full ? 'Explore professionals, providers, and tasks' : 'Popular professional and local services'}</h2></div><button onClick={() => setPage('post-job')} className="text-link">Post a job <ChevronRight size={18} /></button></div>{full && <div className="wide-search"><Search size={20} /><input placeholder="Search web developers, programmers, graphic designers, assistants, errands..." /></div>}<div className="filters">{filters.map(item => <button className={filter === item ? 'active' : ''} onClick={() => setFilter(item)} key={item}>{item}</button>)}</div><div className="gig-grid">{visible.map(gig => <GigCard gig={gig} setPage={setPage} key={gig.title} />)}</div></section>;
}
