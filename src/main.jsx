...
const categories = [
  { name: 'Tech & Web Development', icon: Code2, jobs: '2,400+', hint: 'Web developers, programmers, app builders, automation experts' },
  { name: 'Design & Branding', icon: PenTool, jobs: '1,900+', hint: 'Graphic designers, logo designers, pitch decks, UI/UX, Figma' },
  { name: 'Creative Media', icon: Camera, jobs: '1,180+', hint: 'Video editors, photographers, reels, motion graphics, CGI' },
  { name: 'Marketing & Content', icon: Megaphone, jobs: '970+', hint: 'Social media managers, copywriters, ads, content creators' },
  { name: 'Business & Admin', icon: BriefcaseBusiness, jobs: '1,450+', hint: 'Virtual assistants, admin support, CV writers, data entry' },
  { name: 'Events & Hospitality', icon: UsersRound, jobs: '860+', hint: 'MCs, ushers, DJs, event planners, decorators, staffing' },
...
const gigs = [
  { title: 'Responsive business website with payment-ready landing pages', category: 'Tech & Web Development', location: 'Remote', price: 'From ₦180,000', rating: '4.9', reviews: '121', seller: 'Segun Dev Studio', badge: 'Pro Coder', image: 'linear-gradient(135deg, #0f172a, #38bdf8)' },
  { title: 'Brand identity, logo system, pitch deck styling, and Figma UI kit', category: 'Design & Branding', location: 'Remote', price: 'From ₦95,000', rating: '4.9', reviews: '204', seller: 'Tunde Creative', badge: 'Top Designer', image: 'linear-gradient(135deg, #7c3aed, #22c55e)' },
  { title: '3D motion promo video for fintech, real estate, and product launches', category: 'Creative Media', location: 'Remote', price: 'From ₦280,000', rating: '4.8', reviews: '142', seller: 'MotionLab', badge: 'Pro Seller', image: 'linear-gradient(135deg, #111827, #34d399)' },
  { title: 'Social media manager for content calendar, captions, and weekly reports', category: 'Marketing & Content', location: 'Remote', price: 'From ₦120,000/month', rating: '4.8', reviews: '77', seller: 'Naija Growth Co.', badge: 'Verified Pro', image: 'linear-gradient(135deg, #db2777, #facc15)' },
  { title: 'Virtual assistant for admin, calendar, emails, and research support', category: 'Business & Admin', location: 'Remote', price: 'From ₦90,000/month', rating: '4.7', reviews: '53', seller: 'Nora Desk', badge: 'Pro Seller', image: 'linear-gradient(135deg, #4338ca, #22c55e)' },
  { title: 'Premium corporate MC/host for tech events, galas, and launches', category: 'Events & Hospitality', location: 'Victoria Island, Lagos', price: 'From ₦150,000', rating: '4.9', reviews: '96', seller: 'Chioma N.', badge: 'Verified Host', image: 'linear-gradient(135deg, #92400e, #22c55e)' },
...
const liveTrackingJobs = [
  { task: 'Balogun Market grocery run', provider: 'Amina R.', status: 'In transit', eta: '18 min', lastSeen: 'Updated 42 sec ago', route: 'Lagos Island → Lekki Phase 1', progress: 72 },
  { task: 'CAC document pickup', provider: 'FieldPro', status: 'Arrived at pickup', eta: '35 min', lastSeen: 'Updated 1 min ago', route: 'Ikeja → Yaba', progress: 45 },
  { task: 'Property inspection', provider: 'Deji K.', status: 'On site', eta: 'Proof uploading', lastSeen: 'Live now', route: 'Wuse 2, Abuja', progress: 88 },
];
...
function Hero({ setPage }) {
  return <section className="hero section-shell"><div className="hero-copy"><div className="eyebrow"><ShieldCheck size={18} /> Escrow protected • Verified people • Real-world safety</div><h1>Get trusted help for anything — online or around you.</h1><p>Hire verified professionals and local helpers: programmers, web developers, graphic designers, virtual assistants, event staff, errands, market runs, property help, and everyday services.</p><div className="search-card"><label><Search size={20} /><input placeholder="What do you need help with?" /></label><label><MapPin size={20} /><input placeholder="Where? Lagos, Abuja, Remote..." /></label><button onClick={() => setPage('explore')}>Find Help</button></div><div className="quick-tags">{['Web developer', 'Graphic designer', 'Programmer', 'Market errands', 'Personal assistant', 'Event MC'].map(tag => <button onClick={() => setPage('explore')} key={tag}>{tag}</button>)}</div></div><div className="hero-visual" aria-label="Marketplace preview"><div className="floating-card top-card"><span className="status-dot"></span><div><strong>Escrow funded</strong><small>Provider starts after secure payment</small></div></div><div className="phone-card"><div className="phone-bar"></div><div className="job-preview"><span>Recommended for you</span><h3>Web developer for a Paystack-ready business site</h3><p>Remote • portfolio verified • escrow protected</p><div className="job-meta"><strong>₦180,000</strong><em>4.9 ★</em></div></div><div className="mini-grid"><div><UserCheck /><span>ID check</span></div><div><MessageCircle /><span>Chat</span></div><div><WalletCards /><span>Escrow</span></div><div><BadgeCheck /><span>Proof</span></div></div></div><div className="floating-card bottom-card"><Sparkles size={22} /><div><strong>Gemini AI matching</strong><small>Turn needs into clear job posts</small></div></div></div></section>;
}

function CategoryGrid() { return <section className="section-shell section-block"><div className="section-heading"><span className="kicker">Explore Deur</span><h2>Hire skilled professionals and trusted local helpers.</h2><p>Deur combines Fiverr-style digital talent with real-world services: tech, design, business support, events, errands, delivery, property help, and personal assistance.</p></div><div className="category-grid">{categories.map(({ name, icon: Icon, jobs, hint }) => <article className="category-card" key={name}><div className="icon-wrap"><Icon size={24} /></div><h3>{name}</h3><p>{hint}</p><span>{jobs} active requests</span></article>)}</div><div className="professional-strip"><strong>Professional services on Deur:</strong>{professionalHighlights.map(item => <span key={item}>{item}</span>)}</div></section>; }
...
function Marketplace({ setPage, full = false }) {
  ...
  const filters = ['All', 'Tech & Web Development', 'Design & Branding', 'Creative Media', 'Marketing & Content', 'Business & Admin', 'Events & Hospitality', 'Market Runs', 'Personal Assistant', 'Delivery & Pickup', 'Property Help'];
  const visible = useMemo(() => {
    const q = query.trim().toLowerCase();
    return gigs.filter(gig => {
      const matchesFilter = filter === 'All' || gig.category === filter;
      const haystack = `${gig.title} ${gig.category} ${gig.location} ${gig.seller} ${gig.badge}`.toLowerCase();
      return matchesFilter && (!q || haystack.includes(q));
    });
  }, [filter, query]);
  return <section className="marketplace section-shell section-block"><div className="marketplace-top"><div><span className="kicker">Fiverr-style + local services</span><h2>{full ? 'Explore professionals, providers, and tasks' : 'Popular professional and local services'}</h2></div><button onClick={() => setPage('post-job')} className="text-link">Post a job <ChevronRight size={18} /></button></div>{full && <div className="wide-search"><Search size={20} /><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search web developers, programmers, graphic designers, assistants, errands..." /></div>}<div className="filters">{filters.map(item => <button className={filter === item ? 'active' : ''} onClick={() => setFilter(item)} key={item}>{item}</button>)}</div>{visible.length ? <div className="gig-grid">{visible.map(gig => <GigCard gig={gig} setPage={setPage} key={gig.title} />)}</div> : <div className="empty-state"><h3>No services found</h3><p>Try a different search term or choose another category.</p><button className="btn btn-light" onClick={() => { setFilter('All'); setQuery(''); }}>Reset search</button></div>}</section>;
}


function LiveTracking({ setPage }) {
  return <section className="tracking section-shell section-block"><div className="tracking-panel"><div className="tracking-copy"><span className="kicker">Live location tracking</span><h2>Know where your provider is while real-world work is happening.</h2><p>For errands, delivery, pickup, personal assistant bookings, and property inspections, Deur can show live GPS movement, route progress, ETA, timestamps, and proof updates — with privacy controls and consent.</p><div className="tracking-actions"><button className="btn btn-green" onClick={() => setPage('post-job')}>Post a trackable job</button><button className="btn btn-light" onClick={() => setPage('client-dashboard')}>View tracking dashboard</button></div></div><div className="live-map-card"><div className="map-toolbar"><span><Radio size={16} /> Live GPS active</span><strong>Deur Track</strong></div><div className="map-canvas"><div className="route-line"></div><span className="map-pin start"><MapPin size={18} /></span><span className="map-pin mid"><Truck size={18} /></span><span className="map-pin end"><Check size={18} /></span><div className="pulse-ring"></div></div><div className="tracking-list">{liveTrackingJobs.map(job => <article className="tracking-job" key={job.task}><div className="tracking-job-top"><div><strong>{job.task}</strong><span>{job.provider} • {job.route}</span></div><em>{job.status}</em></div><div className="progress-bar"><span style={{ width: `${job.progress}%` }}></span></div><div className="tracking-meta"><small><Navigation size={14} /> ETA: {job.eta}</small><small>{job.lastSeen}</small></div></article>)}</div></div></div><div className="tracking-features"><article><Route /><h3>Route + ETA</h3><p>Clients see provider movement from pickup to drop-off and get realistic arrival estimates.</p></article><article><Camera /><h3>Proof checkpoints</h3><p>Providers upload photos, receipts, signatures, and completion notes at each stage.</p></article><article><ShieldCheck /><h3>Privacy + safety</h3><p>Live tracking only runs during active jobs and can be reviewed by Deur safety support if there is a dispute.</p></article></div></section>;
}

function Safety() { const items = [['Identity verification','Government ID, profile checks, provider badges, and verification tiers.'],['Escrow-protected payments','Clients fund jobs first. Providers receive payout after completion confirmation.'],['Real-world proof system','Photos, receipts, signatures, timestamps, and location-aware job updates.'],['Safety office','Reports, disputes, emergency contacts, blocked users, and admin review queue.']]; return <section className="safety section-shell section-block"><div className="safety-panel"><div className="section-heading left"><span className="kicker">Built for safety</span><h2>Not just convenience. Trust, life, and property matter.</h2><p>Because Deur includes real-world errands and personal assistance, safety cannot be an afterthought. Live location tracking, proof checkpoints, and consent controls are part of the product.</p></div><div className="safety-list">{items.map(([title, copy]) => <div className="safety-item" key={title}><ShieldCheck /><div><h3>{title}</h3><p>{copy}</p></div></div>)}</div></div></section>; }
...
function PostJobPage({ setPage }) {
  return <section className="form-page section-shell section-block"><div className="form-hero"><span className="kicker">Post a job</span><h1>Tell Deur what you need done.</h1><p>This demo form shows the flow clients will use before Paystack escrow, provider matching, and optional live location tracking.</p></div><form className="big-form" onSubmit={(e)=>{e.preventDefault(); setPage('client-dashboard');}}><label>Job title<input placeholder="e.g. Build a website, design a logo, or buy items from Balogun Market" /></label><label>Category<select><option>Tech & Web Development</option><option>Design & Branding</option><option>Creative Media</option><option>Marketing & Content</option><option>Business & Admin</option><option>Events & Hospitality</option><option>Market Runs</option><option>Personal Assistant</option><option>Delivery & Pickup</option><option>Property Help</option></select></label><label>Location<input placeholder="Lagos Island, Remote, Abuja..." /></label><label>Budget<input placeholder="₦25,000" /></label><label>What should the provider do?<textarea placeholder="Describe the task, proof needed, timing, live tracking requirements, and safety notes."></textarea></label><div className="tracking-toggle"><div><strong>Enable live location tracking</strong><p>Recommended for errands, deliveries, pickup, field work, and personal assistant tasks.</p></div><input type="checkbox" defaultChecked aria-label="Enable live location tracking" /></div><div className="form-actions"><button className="btn btn-green" type="submit"><PlusCircle size={18}/> Post demo job</button><button type="button" onClick={()=>setPage('client-dashboard')} className="btn btn-light">Cancel</button></div></form></section>;
}
function CreateServicePage({ setPage }) {
  return <section className="form-page section-shell section-block"><div className="form-hero"><span className="kicker">I want gigs</span><h1>Create a service people can book.</h1><p>Providers can list programming, web development, graphic design, admin support, event services, errands, personal assistance, field support, and more.</p></div><form className="big-form" onSubmit={(e)=>{e.preventDefault(); setPage('provider-dashboard');}}><label>Service title<input placeholder="e.g. I build websites, design brand identities, or run same-day errands" /></label><label>Category<select><option>Tech & Web Development</option><option>Design & Branding</option><option>Creative Media</option><option>Marketing & Content</option><option>Business & Admin</option><option>Events & Hospitality</option><option>Market Runs</option><option>Personal Assistant</option><option>Delivery & Pickup</option><option>Property Help</option></select></label><label>Service area<input placeholder="Lekki, Lagos Island, Remote..." /></label><label>Starting price<input placeholder="₦8,500" /></label><label>Describe your service<textarea placeholder="Tell clients what you offer, timing, proof, tracking availability, and requirements."></textarea></label><div className="tracking-toggle"><div><strong>I can share live location during active jobs</strong><p>Use only while a booking is active. Helps clients trust errands, deliveries, and field tasks.</p></div><input type="checkbox" defaultChecked aria-label="Provider can share live location" /></div><div className="form-actions"><button className="btn btn-green" type="submit"><PlusCircle size={18}/> Publish demo service</button><button type="button" onClick={()=>setPage('provider-dashboard')} className="btn btn-light">Cancel</button></div></form></section>;
}
function GigDetailPage({ setPage }) {
  return <section className="detail-page section-shell section-block"><button className="text-link" onClick={()=>setPage('explore')}>← Back to explore</button><div className="detail-grid"><div><span className="kicker">Verified service</span><h1>Responsive business website with payment-ready landing pages</h1><p>Remote • portfolio verified • escrow protected • milestone proof required • live tracking available for local jobs</p><div className="work-panel"><h2>What is included</h2><ul><li><Check /> Responsive pages for mobile and desktop.</li><li><Check /> Portfolio review, milestones, and delivery proof.</li><li><Check /> Secure chat before work starts.</li><li><Check /> Escrow payment protection.</li></ul></div></div><aside className="booking-card"><h2>From ₦180,000</h2><p>Client pays safety fee. Provider success fee is calculated by plan.</p><button onClick={()=>setPage('signin')} className="btn btn-green">Continue to booking</button><button onClick={()=>setPage('create-service')} className="btn btn-light">I want gigs like this</button></aside></div></section>;
}

function HomePage({ setPage }) { return <><Hero setPage={setPage} /><CategoryGrid /><Marketplace setPage={setPage} /><LiveTracking setPage={setPage} /><Safety /><HowItWorks /><Pricing setPage={setPage} /><section className="cta section-shell"><div><span className="kicker">Launch direction</span><h2>Start with Lagos/Nigeria. Build trust first. Expand after the model works.</h2><p>Use Paystack subscriptions, escrow-like milestone payments, Supabase authentication, and Gemini AI for matching and job-writing support.</p></div><div className="cta-actions"><button onClick={() => setPage('signup')} className="btn btn-dark">Join Deur</button><button onClick={() => setPage('explore')} className="btn btn-light">Explore services</button></div></section></>; }
...
function App() {
  ...
  const render = () => {
    ...
    if (page === 'tracking') return <LiveTracking setPage={setPage} />;
